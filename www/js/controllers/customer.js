App.controller('CustomerController', function(_, $cordovaCamera, $cordovaOauth, $ionicActionSheet, $ionicLoading, $ionicPopup, $ionicScrollDelegate, $q, $rootScope, $scope, $timeout, $translate, $window, Application, Customer, Dialog, FacebookConnect, SafePopups, HomepageLayout) {
    ...
    $scope.display_login_form = false; //(!$scope.is_logged_in) && (!Customer.display_account_form );
    $scope.display_account_form = true; //$scope.is_logged_in || Customer.display_account_form;
	
	if(Customer.fromLogout){
    ...
    $scope.closeLoginModal = function() {
        //Customer.modal.hide();
		//Customer.modal = null;
		console.log("_.isObject(Customer.modal): " + _.isObject(Customer.modal) + ", _.isFunction(Customer.modal.remove): " + _.isFunction(Customer.modal.remove));
		if (_.isObject(Customer.modal) && _.isFunction(Customer.modal.remove)) {
			Customer.modal.remove();
			Customer.modal = null;
			Customer.showingLogin = false;
		}
		if(Customer.fromLogout == true){
			Customer.loginModal($rootScope);
		}
	...
	$scope.dx={};
    $scope.dx.wrongPassword=false;

    $scope.dx.noEmail=false;
		...
		if($scope.customer.email){ 
			...
			Customer.login($scope.customer).success(function(data) {
				//if(data && data.success) {
					console.log("CUSTOMER - closeLoginModal");
